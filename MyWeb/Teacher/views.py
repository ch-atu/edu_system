...
class Score_edit(TeacherView):
    ...
    def put(self, request):
        query = request.query_params.dict()  # 变成字典
        try:
            Score_instance_queryset = md.ScoreTable.objects.filter(student=query['student'])
            for Score_instance in Score_instance_queryset:
                print(Score_instance.open.course_id)
                if Score_instance.open.course_id == query['course_id']:
                    serializer = serializers.ScoreSerializer(Score_instance, data=request.data, partial=True)
                    if serializer.is_valid():
                        serializer.save()
                        data = {
                            'data': {
