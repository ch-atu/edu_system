...
class SuperUser_create(AdminView):
    def post(self, request):
        ...
        创建SuperUser
        """
        serializer = serializers.SuperUserSerializer(data=request.data, many=True)
...
class SuperUser_edit(AdminView):
    def put(self, request, user_id):
        ...
        """
        try:
            SuperUser_instance = md.User.objects.filter(is_admin=True).get(pk=user_id)
...
class Teacher_create(AdminView):
    ...
    def post(self, request):
        serializer = serializers.TeacherSerializer(data=request.data, many=True)
        if serializer.is_valid():
            serializer.save()
            data = {
                'data': {
...
class Score_create(AdminView):
    ...
    def post(self, request):
        serializer = serializers.ScoreSerializer(data=request.data, many=True, partial=True)
        if serializer.is_valid():
            serializer.save()
            data = {
                'data': {
